...
import { Logger, COLORS } from '../dist/index.js';
 ...
 */
async function animateText(text, color = 'cyan', delay = 30) {
 ...
 */
function drawBox(text, color = 'cyan') {
 ...
 */
async function introduction(logger) {
    clearScreen();
    // Animated logo
    console.log('\n');
 ...
 */
async function chapter1_BasicStyling(logger) {
    clearScreen();
 ...
 */
async function chapter2_RainbowEffects(logger) {
    clearScreen();
    ...
    await sleep(300);
    logger.info('<red>█</><red.dim>█</><yellow.dim>█</><yellow>█</><green>█</><cyan>█</><blue>█</><blue.dim>█</><magenta.dim>█</><magenta>█</>');
    await sleep(300);
    // Mixed styles rainbow
    logger.custom('Mixed Style Rainbow', ['white'], '✨');
    await sleep(300);
    logger.info('<red.bold>B</><yellow.italic>E</><green.underline>A</><cyan.bold>U</><blue.dim>T</><magenta.bold>I</><red.italic>F</><yellow.underline>U</><green.bold>L</>');
    await sleep(500);
    // Animated color wave
    ...
    for (let i = 0; i < 3; i++) {
        ...
        const wave = shifted
            .map(c => `${COLORS[c]}${'█'.repeat(3)}${COLORS.reset}`)
            .join('');
 ...
 */
async function chapter3_RealWorldExamples(logger) {
    clearScreen();
    ...
    for (const req of requests) {
        const methodColor = req.color;
        const statusColor = req.status < 300 ? 'green' : req.status < 500 ? 'yellow' : 'red';
        logger.info(logger.s[methodColor].bold(req.method.padEnd(7)) +
            ' ' +
    ...
    for (const build of buildSteps) {
        const icon = logger.s[build.color](build.icon);
 ...
 */
async function chapter4_DataVisualization(logger) {
    clearScreen();
    ...
    for (const task of tasks) {
        ...
        const bar = logger.s[task.color]('█'.repeat(filled)) + logger.s.gray('░'.repeat(empty));
        const percent = logger.s[task.color].bold(`${task.progress}%`);
 ...
 */
async function chapter5_AdvancedTechniques(logger) {
    clearScreen();
 ...
 * Grand Finale
 */
async function grandFinale(logger) {
    clearScreen();
