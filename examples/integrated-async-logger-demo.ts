...
async function demonstrateIntegratedAsyncLogger() {
  ...
  for (let i = 0; i < 20; i++) {
    promises.push(Promise.resolve(advancedAsyncLogger.info(`High volume message ${i} with PII: john@example.com`)));
    promises.push(Promise.resolve(advancedAsyncLogger.warn(`Warning ${i} with credit card: 4111-1111-1111-1111`)));
    promises.push(Promise.resolve(advancedAsyncLogger.error(`Error ${i} with API key: sk_live_abc123def456`)));
  }
