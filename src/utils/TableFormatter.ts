...
export class TableFormatter {
  ...
  static formatTable(data: unknown, options: TableOptions = {}): string {
    ...
    const validData = data.filter(item => item && typeof item === 'object' && !Array.isArray(item));
  ...
  static format(
    ...
    if (border !== 'none') {
      const topLine =
        borders.topLeft +
        keys
          .map(key => borders.top.repeat((columnWidths[key] ?? 0) + padding * 2))
          .join(borders.topMid) +
        borders.topRight;
      lines.push(
        useColors ? TextStyler.styleParts([[topLine, ...borderColor]], useColors) : topLine
      );
    }

    // Header
    const headerParts = keys.map(
      key => pad + this.padString(key, columnWidths[key] ?? 0, alignment) + pad
    );
    const headerLine =
      (border !== 'none' ? borders.left : '') +
      headerParts.join(border !== 'none' ? borders.middle : ' | ') +
      (border !== 'none' ? borders.right : '');

    if (border !== 'none') {
      const styledBorders = useColors
        ? TextStyler.styleParts([[borders.left, ...borderColor]], useColors)
        : borders.left;
      const styledMiddle = useColors
        ? TextStyler.styleParts([[borders.middle, ...borderColor]], useColors)
        : borders.middle;
      const styledRight = useColors
        ? TextStyler.styleParts([[borders.right, ...borderColor]], useColors)
        : borders.right;
      const styledHeader = useColors
        ? TextStyler.styleParts([[headerParts.join(styledMiddle), ...headerColor]], useColors)
        : headerParts.join(borders.middle);
      lines.push(styledBorders + styledHeader + styledRight);
    } else {
      lines.push(
        useColors ? TextStyler.styleParts([[headerLine, ...headerColor]], useColors) : headerLine
      );
    }
    ...
    if (border !== 'none') {
      const sepLine =
        borders.leftMid +
        keys
          .map(key => borders.mid.repeat((columnWidths[key] ?? 0) + padding * 2))
          .join(borders.midMid) +
        borders.rightMid;
      lines.push(
        useColors ? TextStyler.styleParts([[sepLine, ...borderColor]], useColors) : sepLine
      );
    }
    ...
    if (border !== 'none') {
      const bottomLine =
        borders.bottomLeft +
        keys
          .map(key => borders.bottom.repeat((columnWidths[key] ?? 0) + padding * 2))
          .join(borders.bottomMid) +
        borders.bottomRight;
      lines.push(
        useColors ? TextStyler.styleParts([[bottomLine, ...borderColor]], useColors) : bottomLine
      );
    }
  ...
  static box(
    ...
    const { border = 'single', color = [], borderColor = ['dim'], padding = 1 } = options;
    ...
    lines.forEach(line => {
      ...
      const styledLine =
        color.length && useColors
          ? TextStyler.styleParts([[paddedLine, ...color]], useColors)
          : paddedLine;
  ...
  static list(
    ...
    const { bullet = '•', indent = 0, bulletColor = [], itemColor = [] } = options;

    const indentStr = ' '.repeat(indent);

    return items.map(item => {
      const styledBullet =
        bulletColor.length && useColors
          ? TextStyler.styleParts([[bullet, ...bulletColor]], useColors)
          : bullet;
      const styledItem =
        itemColor.length && useColors
          ? TextStyler.styleParts([[item, ...itemColor]], useColors)
          : item;
