...
export abstract class CustomFormatter implements ICustomFormatter {
  ...
  protected escape(str: string): string { return str; }
...
export class FunctionFormatter extends CustomFormatter {
  ...
  constructor(fn: (entry: LogEntry) => string | Buffer) { super(); this.fn = fn; }
  format(entry: LogEntry): string | Buffer { return this.fn(entry); }
}
