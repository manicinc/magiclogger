...
### Structured Logging with NDJSON Support
MagicLogger supports **NDJSON (Newline Delimited JSON)** format, similar to Pino and other modern logging libraries. Each log entry is a complete JSON object on its own line, making it perfect for:
- Stream processing and real-time log analysis
- Log aggregation systems (ELK stack, Splunk, DataDog)
- Easy parsing without loading entire files
