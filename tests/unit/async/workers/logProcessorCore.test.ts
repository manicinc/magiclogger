...
import { createInitialState, formatEntries, prepareFileData, batchForNetwork, processLogs, updateConfig, type WorkerConfig, type WorkerState } from '../../../../src/async/workers/log-processor-core';
import type { LogEntry } from '../../../../src/types/transport';

// Minimal stub of LogEntry fulfilling required fields for processor
type TestEntry = Pick<LogEntry, 'timestamp' | 'timestampMs' | 'level' | 'message' | 'metadata' | 'tags' | 'context' | 'id'>;
...
describe('worker wrapper handleWorkerMessage', () => {
  ...
  interface ProcessedMessage { type: string; [k: string]: unknown }
  // Outbound messages to worker
  interface ConfigMessage { type: 'config'; config: Partial<WorkerConfig> }
  interface LogsMessage { type: 'logs'; entries: TestEntry[] }
  ...
  beforeAll(async () => {
    (globalThis as unknown as Record<string, unknown>).self = {
      postMessage: (msg: ProcessedMessage) => { posted.push(msg); },
      addEventListener: () => { /* listener stub */ return undefined; },
      close: () => { /* close stub */ return undefined; },
    } as unknown;
