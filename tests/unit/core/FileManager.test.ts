...
describe('FileManager', () => {
  ...
  describe('resolveLogDir method', () => {
    ...
    it('should throw TypeError for non-string input', () => {
      expect(() => fileManager.resolveLogDir(123 as unknown as string))
        .toThrow(TypeError);
      expect(() => fileManager.resolveLogDir(123 as unknown as string))
    ...
    it('should throw TypeError for null input', () => {
      expect(() => fileManager.resolveLogDir(null as unknown as string))
        .toThrow(TypeError);
    });

    it('should throw TypeError for undefined input', () => {
      expect(() => fileManager.resolveLogDir(undefined as unknown as string))
        .toThrow(TypeError);
    });

    it('should throw TypeError for object input', () => {
      expect(() => fileManager.resolveLogDir({} as unknown as string))
        .toThrow(TypeError);
    });
  ...
  describe('initLogFileSync method (synchronous)', () => {
    ...
    it('should throw error on failure', () => {
      ...
      expect(() => fileManager.initLogFileSync())
  ...
  describe('cleanupOldLogs method', () => {
    it('should remove files older than retention period', async () => {
      ...
      fsMocks.statSync
        .mockReturnValueOnce(createStatsMock({ 
          isDirectory: false, 
          mtimeMs: oldFileTime 
        }))
        .mockReturnValueOnce(createStatsMock({ 
    ...
    it('should process multiple files correctly', async () => {
      ...
      fsMocks.existsSync.mockReturnValue(true);
      fsMocks.readdirSync.mockReturnValue(['very-old.log', 'old.log', 'recent.log'] as unknown as fs.Dirent[]);
