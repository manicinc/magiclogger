...
describe('NodeLogger', () => {
  ...
  describe('constructor and initialization', () => {
    ...
    it('should initialize file manager when writeToDisk is true', () => {
      ...
      console.log(
        'MockedFileManager mock calls before:',
        MockedFileManager.mock?.calls?.length || 'no mock'
      );

      new NodeLogger({ writeToDisk: true });

      console.log(
        'MockedFileManager mock calls after:',
        MockedFileManager.mock?.calls?.length || 'no mock'
      );
      console.log(
        'MockedFileManager mock instances:',
        MockedFileManager.mock?.instances?.length || 'no mock'
      );
  ...
  describe('basic logging methods', () => {
    it('should log info messages', () => {
      ...
      expect(mockFormatter.preserveLinks).toHaveBeenCalledWith(
  ...
  describe('custom logging method', () => {
    ...
    it('should write to file when enabled', () => {
      const fileLogger = new NodeLogger({ writeToDisk: true });
      // Ensure ready
      mockFileManager.isReady.mockReturnValue(true);
  ...
  describe('header method', () => {
    ...
    it('should create header with custom colors', () => {
      ...
      expect(mockFormatter.colorize).toHaveBeenCalledWith(expect.any(String), ['red', 'bgWhite']);
      expect(consoleLogSpy).toHaveBeenCalled();
    });
  ...
  describe('table method', () => {
    ...
    it('should write table summary to file', () => {
      ...
      expect(mockFileManager.appendToFile).toHaveBeenCalledWith(expect.stringContaining('Row 1:'));
      expect(mockFileManager.appendToFile).toHaveBeenCalledWith(expect.stringContaining('Row 2:'));
    });
  ...
  describe('link method', () => {
    it('should format and display links', () => {
      ...
      expect(mockFormatter.colorize).toHaveBeenCalledWith('[Example Site]', [
        'brightCyan',
        'underline',
      ]);
      expect(consoleLogSpy).toHaveBeenCalled();
    ...
    it('should use URL as label when description not provided', () => {
      ...
      expect(mockFormatter.colorize).toHaveBeenCalledWith('[https://example.com]', [
        'brightCyan',
        'underline',
      ]);
      expect(consoleLogSpy).toHaveBeenCalled();
    ...
    it('should write link to file', () => {
      ...
      expect(mockFileManager.appendToFile).toHaveBeenCalledWith('Example: https://example.com');
    });
  });
  ...
  describe('progressBar method', () => {
    ...
    it('should handle 100% progress', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalled();
      expect(mockFileManager.appendToFile).toHaveBeenCalledWith('[PROGRESS] 100% complete');
    });
  ...
  describe('color method', () => {
    ...
    it('should handle multiple colors', () => {
      ...
      expect(mockFormatter.colorize).toHaveBeenCalledWith('test', ['red', 'bold', 'underline']);
    });
  });
