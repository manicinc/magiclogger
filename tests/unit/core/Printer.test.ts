...
describe('Printer', () => {
  ...
  let consoleTableSpy: jest.SpyInstance<
    void,
    [tabularData?: unknown, properties?: readonly string[] | undefined]
  >;
  let consoleErrorSpy: jest.SpyInstance<void, [message?: unknown, ...optionalParams: unknown[]]>;
  let consoleClearSpy: jest.SpyInstance<void, []>;
  ...
  beforeEach(() => {
    ...
    // Configure Printer to use a full console shape backed by spies
    const mockConsole: Console = {
      ...console,
      log: consoleLogSpy as unknown as Console['log'],
      table: consoleTableSpy as unknown as Console['table'],
      error: consoleErrorSpy as unknown as Console['error'],
      clear: consoleClearSpy as unknown as Console['clear'],
    };
  ...
  describe('print method', () => {
    ...
    it('should print to console with styling in browser environment', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith('%cbrowser message', 'font-family: monospace;');
    });
  ...
  describe('printProgress method', () => {
    ...
    it('should update progress inline in terminal (production)', () => {
      ...
      const calls = processStdoutWriteSpy.mock.calls as unknown as [string, ...unknown[]][];
      expect(calls.some(call => (call[0] as string).includes('\r'))).toBe(true);
      expect(calls.some(call => (call[0] as string).includes('60%'))).toBe(true);
    });
    ...
    it('should show speed when current and total provided', () => {
      ...
      const hasSpeed = calls.some(call => typeof call[0] === 'string' && call[0].includes('/s'));
    ...
    it('should add newline at 100% completion in terminal', () => {
      ...
      const calls = processStdoutWriteSpy.mock.calls;
      const hasNewline = calls.some(call => typeof call[0] === 'string' && call[0].includes('\n'));
  ...
  describe('printTable method', () => {
    ...
    it('should not truncate when truncate is false', () => {
      const longData = [{ text: 'This text should not be truncated even though it is long' }];
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle table with very wide columns', () => {
      const wideData = [{ veryLongColumnName: 'a'.repeat(200) }];
